	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					...
					url              : getPath()+"/POCO/product-preview.action",  // 上传文件的路径
...
/*		            html += '					<div class="upload_btn"><a href="'+getPath()+'/POCO/views/success.jsp">开始上传</a></div>';
*/		            html += '				</div>';
		            html += '			</div>';
			...
			this.addEvent = function(){
				...
				$(".upload_btn").bind("click", function(e){
					...
					if(ZYFILE.funReturnNeedFiles().length > 0){
						//$("#fileSubmit").click();
						window.location = getPath()+"/POCO/views/success.jsp";
					}else{
						alert("请先选中文件再点击上传");
