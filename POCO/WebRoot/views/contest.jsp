...
function getAllContest(){
  $.post(getRootPath()+"/vistor/getAllContests.action", function(data){
	if(data.returnCode == '00'){
		var html = "";
		for(var i = 0; i < data.contestInfo.length; i++){
			var startTime = Date.parse(new Date(data.contestInfo[i].startTime));
			var endTime = Date.parse(new Date(data.contestInfo[i].endTime));
			...
			html = "<li><h2>"+data.contestInfo[i].contestName+curStatus+"</h2>"
					+  "<p>" + data.contestInfo[i].contestDesc +"</p>"
					+  "<div class='joinBtn'><a href='"+getRootPath()+"/views/contestInfo.jsp?contestId="+ data.contestInfo[i].contestId+"')'>点击进入</a></li>";	
